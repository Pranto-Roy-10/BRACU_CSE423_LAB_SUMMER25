...
FP_HIT_RADIUS = 60.0   # first-person collider 
...
def bullet_hits_wall(b):
    margin = BULLET_SIZE * 0.5
    return (abs(b['x']) >= GRID_LENGTH - margin) or (abs(b['y']) >= GRID_LENGTH - margin)
...
def update_state():
    ...
    for i, b in enumerate(bullets):
        ...
        for ei, e in enumerate(enemies):
            if dist_xy(b['x'], b['y'], e['x'], e['y']) < (enemy_current_radius(e) + BULLET_SIZE * 0.6):
                rm.append(i)
                score += 1
                enemies[ei] = spawn_enemy()
                break

    for i in sorted(set(rm), reverse=True):
        bullets.pop(i)
    ...
    # Cheat: 
    if cheat_mode:
        player_angle_deg -= CHEAT_SPIN_SPEED  
        # find nearest enemy
